...
- 支持 ZIP 打包下载或逐张原图下载。
- 支持选择本地保存文件夹；不支持直接选文件夹时，可在下载时弹出保存位置选择框。
...
当前仓库对应的最新发布包示例：`PixivDL-Browser-0.5.5.zip`。
...
5. 在右侧选择 `ZIP` 或 `文件` 模式后点击下载；需要改保存目录时展开“更多设置”。
...
下载面板默认只露出模式、下载按钮和进度；保存目录、询问保存位置和代理在“更多设置”里。点击“浏览”会优先请求选择一个本地保存文件夹；授权后 ZIP 和文件模式会直接写入该文件夹。如果当前扩展环境不支持直接选择文件夹，按钮会改为启用“下载时询问保存位置”，下载时由浏览器弹出保存对话框。
未选择本地文件夹且未启用询问时，下载仍由浏览器处理，默认保存到浏览器下载目录下的 `PixivDL` 子目录；“默认”按钮会取消本地文件夹授权并恢复浏览器下载目录模式。
