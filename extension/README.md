...
也可以加载版本目录 `D:\pixivdl\dist\PixivDL-Browser-0.5.5`，但后续需要继续覆盖同一个目录并刷新扩展，避免 Chrome 把新目录识别为另一个扩展。
...
5. 在右侧选择 ZIP 或文件模式后点击下载；需要改保存目录时展开“更多设置”。
6. 需要浏览收藏图库时，切换到收藏夹并点击“新标签浏览”。

下载面板默认只露出模式、下载按钮和进度；保存目录、询问保存位置和代理在“更多设置”里。点击“浏览”会优先请求选择一个本地保存文件夹；授权后 ZIP 和文件模式会直接写入该文件夹。如果当前扩展环境不支持直接选择文件夹，会改为启用“下载时询问保存位置”，下载时由浏览器弹出保存对话框。未选择本地文件夹且未启用询问时，下载仍由浏览器处理，并可在下载面板中改为浏览器下载目录下的其他相对目录；“默认”按钮会恢复浏览器下载目录模式。本地代理设置只作用于当前浏览器配置文件。
