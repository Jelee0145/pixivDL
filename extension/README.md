...
也可以加载版本目录 `D:\pixivdl\dist\PixivDL-Browser-0.5.0`，但后续需要继续覆盖同一个目录并刷新扩展，避免 Chrome 把新目录识别为另一个扩展。
...
保存目录可在下载面板中改为浏览器下载目录下的其他相对目录；本地代理设置只作用于当前浏览器配置文件。

如果接口获取失败，空状态区会显示“前往 Pixiv 查看”，用于打开当前 PID 的 Pixiv 原作品页。
普通弹窗切换到收藏夹后，顶部输入框会改为本地收藏搜索，支持按标题、作者和 PID 筛选；新标签收藏仓库也有独立搜索框，点击封面会在右侧显示作品详情，并支持导入/导出收藏 JSON。收藏卡片和详情栏都可以直接下载作品，下载前会弹窗选择 ZIP 或文件模式。
